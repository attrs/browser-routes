...
var debug = config('x-router.debug') === 'true' ? true : false;
  ...
  var body = function Router(req, res, onext) {
    ...
    var forward = function(err) {
      ...
      if( route.fn.__router__ ) {
        url = req.url = div.sub;
        parentURL = req.parentURL = path.join(oParentURL, div.parent);
      }
...
(function() {
  ...
  function bootup() {
    ...
    // observe anchor tags
    if( config('x-router.observe') !== 'false' ) {
