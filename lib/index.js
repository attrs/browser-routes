...
function Application() {
  ...
  router.href = function(href, body, options) {
    ...
    response = router.response = {
      render: function(src, options, odone) {
        if( arguments.length == 2 && typeof options === 'function' ) odone = options, options = null;
        var done = function(err, result) {
          if( err ) return odone ? odone.call(router, err) : console.error(err);
          if( odone ) odone.call(this, null, target, result);
        };
        ...
        if( !target ) return done(new Error('missing options.target'));
        
        if( typeof target === 'string' ) {
          target = document.querySelector(target);
          if( !target ) return done(new Error('target not found:' + (o.target || config['view target'])));
        }
        o.target = target;
        ...
        if( router.fire('beforerender', {
          ...
          engine.call(router, src, o, function(err, result) {
...
Application.engine('default', function defaultRenderer(src, options, done) {
  ...
  var render = function(err, contents) {
    if( err ) return done(err);
    if( typeof contents === 'string' ) contents = app.util.evalhtml(contents);
  ...
  if( typeof src === 'string' ) app.util.ajax(src, render);
