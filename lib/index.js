...
var debug = config('xrouter.debug') === 'true' ? true : false;
var routermark = {};
...
function Router(id) {
  ...
  var body = function Router(req, res, onext) {
    ...
    var forward = function(err) {
      ...
      if( typeof fn == 'string' ) {
        fn = fn.trim();
        if( !fn.indexOf('/') || !fn.indexOf('..') ) return res.redirect(path.resolve(req.parentURL, fn));
        else {
...
function Application(options) {
  ...
  router.fullhref = function(url) {
    ...
    else if( !lasthref ) url = baseURL + '/' + url;
    else if( lasthref ) url = path.dirname(lasthref) + '/' + url;
  ...
  router.href = function(href, body, options) {
    ...
    if( debug ) console.info('href', lasthref, href);
    ...
    if( router.request && router.request.href === parsed.fullpath ) return;
    if( options.writestate !== false ) referer = lasthref, lasthref = parsed.fullpath;
...
(function() {
  var mode = config('xrouter.mode') || (history.pushState ? 'pushstate' : 'hash');
  ...
  if( mode === 'pushstate' ) {
    ...
    addEventListener(window, 'hashchange', function() {
      ...
      if( location.hash.length >= 1 ) Application.href(location.hash.substring(1));
    });
  ...
  function bootup() {
    ...
    // observe anchor tags
    if( config('xrouter.observe') !== 'false' ) {
